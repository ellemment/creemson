...
let processorPromise: ReturnType<typeof getProcessor> | null = null;
...
async function getMarkdownTutPage(filename: string): Promise<MarkdownTutPage> {
  ...
  let page = await processMarkdown(processor, file);
...
async function processMarkdown(
  ...
  function addSlide() {
    let sequence = root[root.length - 1] as SequenceNode;
